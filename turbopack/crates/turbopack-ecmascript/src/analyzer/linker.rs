...
use super::{JsValue, arena::Arena, graph::VarGraph};
use crate::analyzer::arena::BumpVec;
        ...
        match step {
            ...
            Step::LeaveVar(var) => {
                ...
                if cycle_stack.is_empty() && fun_args_values.lock().is_empty() {
                    var_cache
                        .lock()
                        .insert(var, done.last().unwrap().clone_in(arena));
                }
            }
