...
pub mod test_utils {
    ...
    use crate::{
        analyzer::{
            RequireContextValue, arena::Arena, builtin::replace_builtin, imports::ImportAttributes,
    ...
    pub async fn visitor<'a>(
        ...
        let mut new_value = match v {
            ...
            _ => {
                ...
                let m2 = replace_builtin(arena, &mut v);
                let m = m1 || m2 || v.make_nested_operations_unknown();
                return Ok((v, m));
            }
        };
        new_value.normalize_shallow(arena);
        Ok((new_value, true))
    }
}
...
mod tests {
    ...
    use crate::{
        AnalyzeMode,
        analyzer::{arena::Arena, graph::AssignmentScopes, imports::ImportAttributes},
    };
    ...
    async fn fixture_op(input: RcStr) -> anyhow::Result<()> {
        ...
        let mut named_values = var_graph
            ...
            .map(|((id, ctx), value)| {
                ...
                let value = value.clone_in(&arena);
                if unique {
                ...
                let steps = match effect {
                    ...
                    Effect::Call {
                        ...
                        resolved.push((
                            ...
                            if new {
                                JsValue::new_from_iter(&arena, func, new_args)
                            } else {
                                JsValue::call_from_iter(&arena, func, new_args)
                            },
                        ));
                        steps
                    }
                    ...
                    Effect::MemberCall {
                        ...
                        resolved.push((
                            ...
                            JsValue::member_call_from_iter(&arena, obj, prop, new_args),
                        ));
